...
connection, instead of every client spawning its own Telethon session â€”
...
machine â€” the endpoint is unauthenticated, so never publish it on a public
interface.
...
logged** â€” set `TELEGRAM_AUDIT_LOG_ARGS=1` to also record parameter *names*.
...
- [chigwell/telegram-mcp](https://github.com/chigwell/telegram-mcp) â€” upstream project this fork is built on top of
