...
def test_audit_records_duration(tmp_path, monkeypatch):
    path = tmp_path / "audit.log"
    monkeypatch.setenv("TELEGRAM_AUDIT_LOG", str(path))

    audit.record_audit(tool_name="send_message", ok=True, duration_ms=12.3456)

    data = _read_lines(path)[0]
    assert data["duration_ms"] == 12.346  # rounded to 3 decimals


def test_audit_omits_duration_when_not_provided(tmp_path, monkeypatch):
    path = tmp_path / "audit.log"
    monkeypatch.setenv("TELEGRAM_AUDIT_LOG", str(path))

    audit.record_audit(tool_name="t", ok=True)

    assert "duration_ms" not in _read_lines(path)[0]


@pytest.mark.asyncio
async def test_with_account_records_duration(tmp_path, monkeypatch):
    monkeypatch.setenv("TELEGRAM_AUDIT_LOG", str(tmp_path / "audit.log"))
    ...
    await fake_write(chat_id=5)

    line = _read_lines(tmp_path / "audit.log")[0]
